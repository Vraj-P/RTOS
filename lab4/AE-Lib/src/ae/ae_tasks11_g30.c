...
{   
    ...
    return_msg = tsk_create(&g_tids[1], &task1, HIGH, 0x200);	
    ...
    return_msg = tsk_create(&g_tids[3], &task3, HIGH, 0x200);
    if(return_msg == RTX_ERR)
        failed_tests += 1;
    else
        passed_tests += 1;
...
{   
    ...
    if(mbx_id < 0)
    {
        printf("Failed to create a mailbox\r\n");
        failed_tests += 1;
        tsk_exit();
    }
    else
