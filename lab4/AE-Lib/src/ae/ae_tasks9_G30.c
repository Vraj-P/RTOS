...
{   
    ...
    mbx_t mbx_id = mbx_create(BUF_LEN);
    if(mbx_id < 0)
    {
        failed_tests += 1;
        tsk_exit();
    ...
    {   
        ...
        ptr->type = DEFAULT;
        ptr->sender_tid = g_tids[1];
        buf += msg_hdr_size;                        
        *buf = 'A';
    
        int ret_val = send_msg_nb(g_tids[2], (void *)ptr);
        if(ret_val == RTX_ERR)
        {
            printf("failed send, i = %d\r\n", i);
        }

        tsk_yield();
        printf("in task 1\r\n");

        ret_val = recv_msg_nb(g_buf_return1, BUF_LEN);
        if(ret_val == RTX_ERR)
        {
            printf("failed receive, i = %d\r\n", i);
        }


        if(ret_val == RTX_ERR)
        {
            printf("ret_val error task 1, i = %d\r\n", i);
            failed_tests += 1;
            // tsk_exit();
        }
        else
...
{   
    ...
    {   
        ...
        *buf = 'B';
        ...
        int ret_val = send_msg_nb(g_tids[1], (void *)ptr);
        if(ret_val == RTX_ERR)
        {
            printf("failed send, i = %d\r\n", i);
        }
        ...
        ret_val = recv_msg_nb(g_buf_return2, BUF_LEN);
        if(ret_val == RTX_ERR)
        {
            printf("failed receive, i = %d\r\n", i);
        }
        ...
        if(ret_val == RTX_ERR)
        {
            printf("ret_val error task 2, i = %d\r\n", i);
            failed_tests += 1;
            // tsk_exit();
        }
        else
